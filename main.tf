...
resource "aws_acm_certificate" "server_vpn_cert" {
  certificate_body  = file("${path.module}/${var.server_cert}")
  private_key       = file("${path.module}/${var.server_private_key}")
  certificate_chain = file("${path.module}/${var.ca_crt}")
}

resource "aws_acm_certificate" "client_vpn_cert" {
  certificate_body  = file("${path.module}/${var.client_cert}")
  private_key       = file("${path.module}/${var.client_private_key}")
  certificate_chain = file("${path.module}/${var.ca_crt}")
}
